...
public abstract class AbstractID3v2FrameData {
    ...
    protected void synchroniseAndUnpackFrameData(byte[] bytes, String preferredLanguage) throws InvalidDataException {
        if (unsynchronisation && BufferTools.sizeSynchronisationWouldSubtract(bytes) > 0) {
            ...
            unpackFrameData(synchronisedBytes, preferredLanguage);
        } else {
            unpackFrameData(bytes, preferredLanguage);
        }
    }
    ...
    protected abstract void unpackFrameData(byte[] bytes, String preferredLanguage) throws InvalidDataException;

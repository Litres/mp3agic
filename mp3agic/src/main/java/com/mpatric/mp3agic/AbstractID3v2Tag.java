...
public abstract class AbstractID3v2Tag implements ID3v2 {
    ...
    public AbstractID3v2Tag(String preferredLang) {
        frameSets = new TreeMap<String, ID3v2FrameSet>();
        this.preferredLang = preferredLang;
    }

    public AbstractID3v2Tag(String preferredLang, byte[] bytes) throws NoSuchTagException, UnsupportedTagException, InvalidDataException {
        this(preferredLang, bytes, false);
    }

    public AbstractID3v2Tag(String preferredLang, byte[] bytes, boolean obseleteFormat) throws NoSuchTagException, UnsupportedTagException, InvalidDataException {
        this.preferredLang = preferredLang;
        frameSets = new TreeMap<String, ID3v2FrameSet>();
    ...
    public void setUrl(String url) {
        if (url != null && url.length() > 0) {
            invalidateDataLength();
            ID3v2UrlFrameData frameData = new ID3v2UrlFrameData(null, useFrameUnsynchronisation(), url);
    ...
    public byte[] getAlbumImage() {
        ID3v2PictureFrameData frameData = createPictureFrameData(obseleteFormat ? ID_IMAGE_OBSELETE : ID_IMAGE, preferredLang);
    ...
    private ArrayList<ID3v2ChapterFrameData> extractChapterFrameData(String id) {
        ...
        if (frameSet != null) {
            ...
            for (ID3v2Frame frame : frames) {
                ...
                try {
                    frameData = new ID3v2ChapterFrameData(useFrameUnsynchronisation(), frame.getData(), preferredLang);
    ...
    protected ID3v2TextFrameData extractTextFrameData(String id) {
        ...
        if (frameSet != null) {
            ...
            try {
                frameData = new ID3v2TextFrameData(useFrameUnsynchronisation(), frame.getData(), preferredLang);
                return frameData;
            } catch (InvalidDataException e) {
                // do nothing
            }
    ...
    private ID3v2WWWFrameData extractWWWFrameData(String id) {
        ...
        if (frameSet != null) {
            ...
            try {
                frameData = new ID3v2WWWFrameData(useFrameUnsynchronisation(), frame.getData(), preferredLang);
                return frameData;
            } catch (InvalidDataException e) {
                // do nothing
            }
    ...
    private ID3v2UrlFrameData extractUrlFrameData(String id) {
        ...
        if (frameSet != null) {
            ...
            try {
                frameData = new ID3v2UrlFrameData(useFrameUnsynchronisation(), frame.getData(), preferredLang);
                return frameData;
            } catch (InvalidDataException e) {
                // do nothing
            }
    ...
    private ID3v2CommentFrameData extractCommentFrameData(String id, boolean itunes) {
        ...
        if (frameSet != null) {
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    frameData = new ID3v2CommentFrameData(useFrameUnsynchronisation(), frame.getData(), preferredLang);
    ...
    private ID3v2PictureFrameData createPictureFrameData(String id, String preferredLang) {
        ID3v2FrameSet frameSet = frameSets.get(id);
        if (frameSet != null) {
            ID3v2Frame frame = (ID3v2Frame) frameSet.getFrames().get(0);
            ...
            try {
                if (obseleteFormat)
                    frameData = new ID3v2ObseletePictureFrameData(useFrameUnsynchronisation(), frame.getData(), preferredLang);
                else
                    frameData = new ID3v2PictureFrameData(useFrameUnsynchronisation(), frame.getData(), preferredLang);
                return frameData;
            } catch (InvalidDataException e) {
                // do nothing
            }
